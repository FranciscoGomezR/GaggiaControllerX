...
pumpCtrl_status_t init_pump_controller(void)
{
  ...
  sPumpProfData.Pwr_2ndP         = (uint16_t)BREW_PWR_DEFAULT;
  sPumpProfData.Pwr_1stP         = (uint16_t)PRE_INFUSE_PWR_DEFAULT;
  sPumpProfData.Pwr_3rdP         = (uint16_t)TAPER_PWR_DEFAULT;
  sPumpProfData.Time_1stP_ms     = (uint16_t)PRE_INFUSE_TIME_MSECS_DEFAULT;
  sPumpProfData.Time_2ndP_ms     = (uint16_t)BREW_TIME_MSECS_DEFAULT;
  sPumpProfData.Time_3rdP_ms     = (uint16_t)TAPER_TIME_MSECS_DEFAULT;
  sPumpProfData.Time_Rampup_ms   = (uint16_t)RAMP_UP_TIME_MSECS_DEFAULT;
  sPumpProfData.Time_Rampdown_ms = (uint16_t)RAMP_DOWN_TIME_MSECS_DEFAULT;
      ...
      case ST_TIMER:
        if(-- g_Pump_param_s.Counter != 0 )
        {
          ...
          if( g_Pump_param_s.Slope != 0)
          {
            ...
            /*pump_ssr_pwr_update(g_Pump_param_s.PumpPower);*/
            pump_ssr_pwr_update(g_Pump_param_s.PumpPower);
          }else{}
...
static void load_pump_param(pump_profile_data_t *ptr_profile_param, pump_param_t *ptr_pump_param)
{
  ...
  ptr_pump_param->RampUp_tCounts   = (uint16_t)(ptr_profile_param->Time_Rampup_ms / PUMP_BASE_TIME_MSECS);
  /* Counts (time) from step 4 to 5 */
  /* Counts (time) from step 6 to 7 */
  ptr_pump_param->RampDown_tCounts = (uint16_t)(ptr_profile_param->Time_Rampdown_ms / PUMP_BASE_TIME_MSECS);
  /* Counts (time) from step 1 to 2 */
  ptr_pump_param->tCounts_1stP     = (uint16_t)(ptr_profile_param->Time_1stP_ms / PUMP_BASE_TIME_MSECS);
  /* Counts (time) from step 3 to 4 */
  ptr_pump_param->tCounts_2ndP     = (uint16_t)(ptr_profile_param->Time_2ndP_ms / PUMP_BASE_TIME_MSECS);
  /* Counts (time) from step 5 to 6 */
  ptr_pump_param->tCounts_3rdP     = (uint16_t)(ptr_profile_param->Time_3rdP_ms / PUMP_BASE_TIME_MSECS);
